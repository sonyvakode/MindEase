...
export default function Appointments({ onNavigate }: { onNavigate?: (tab: string, state?: any) => void }) {
  ...
  const handleBook = async () => {
    if (!selectedSpecialist) {
      alert("Please choose a counsellor from the list of doctors first.");
      return;
    }
    if (!selectedTime) {
      alert("Please choose an available slot time from the list.");
      return;
    }
  ...
  const handleJoinSession = (appt: Appointment) => {
    ...
    onNavigate?.("chatbot", { specialist: specialistInfo, bookedAdvisorMode: true });
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center space-y-6">
        <motion.div 
          initial={{ scale: 0 }} 
          animate={{ scale: 1 }} 
          className="w-20 h-20 bg-indigo-500/20 rounded-full flex items-center justify-center text-indigo-400"
        >
        ...
        <p className="text-slate-500 dark:text-white/40 max-w-md font-medium leading-relaxed">
          Your session with {selectedSpecialist?.name} has been scheduled for May {selectedDate} at {selectedTime}.
        </p>
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <button 
            onClick={() => {
              if (selectedSpecialist) {
                onNavigate?.("chatbot", { specialist: selectedSpecialist, bookedAdvisorMode: true });
                setBooked(false);
              }
            }}
            className="purple-gradient px-8 py-3.5 rounded-xl font-bold text-white shadow-xl shadow-indigo-500/20 transition-all hover:scale-105 active:scale-95 flex items-center gap-2 cursor-pointer"
          >
          ...
          <button 
            ...
            className="bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 px-8 py-3.5 rounded-xl font-bold text-slate-700 dark:text-white hover:bg-slate-200 dark:hover:bg-white/10 transition-all active:scale-95 cursor-pointer"
          >
            Book Another
          </button>
        </div>
    ...
    <div className="space-y-8 pb-12">
          ...
          >
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
              {myAppointments.map((appt) => (
                <GlassCard key={appt.id} className="min-w-[320px] p-4 flex items-center gap-4 border-indigo-500/20 bg-indigo-500/5 group relative">
                  <div className="w-12 h-12 bg-white/5 rounded-xl flex items-center justify-center">
                    <Video className="w-6 h-6 text-indigo-400 shadow-indigo-500/20" />
                  </div>
                  <div className="flex-1">
                    ...
                    {appt.status === 'confirmed' && (
                      <button 
                        ...
                        className="w-full mt-2 bg-indigo-500/20 hover:bg-indigo-500 border border-indigo-500/30 py-2 rounded-lg text-[9px] font-bold text-indigo-400 hover:text-white uppercase tracking-widest transition-all flex items-center justify-center gap-2"
                      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {specialists.map((s, i) => {
                ...
                return (
                  <motion.div
                    ...
                    className={cn(
                      "glass-card p-4 pb-6 cursor-pointer group hover:border-indigo-500/50 shadow-2xl transition-all duration-300 relative",
                      isSelected ? "border-indigo-500 bg-indigo-500/5 ring-1 ring-indigo-500/30" : ""
                    )}
                  >
                    <div className="relative mb-5">
                      {isSelected && (
                        <div className="absolute top-2 left-2 px-2.5 py-1 bg-indigo-500 rounded-lg flex items-center gap-1 border border-indigo-400/50 shadow-lg z-20">
                      ...
                      <button 
                        ...
                        className="absolute bottom-2 right-2 w-8 h-8 bg-white/10 hover:bg-indigo-500 backdrop-blur-md rounded-lg flex items-center justify-center border border-white/10 transition-all group-hover:scale-110 active:scale-95"
                      >
                        <Info className="w-4 h-4 text-white" />
                      </button>
                    </div>
                    <div className="text-center">
                      <h3 className="font-bold text-sm mb-1 group-hover:text-indigo-400 transition-colors uppercase tracking-tight">{s.name}</h3>
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
             <GlassCard className="p-6 border-white/5">
                <div className="flex items-center justify-between mb-8">
                   ...
                   <div className="flex items-center gap-3">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-indigo-400">May Journey</span>
                      ...
                      return (
                        <button 
                         ...
                         className={cn(
                           ...
                           isSelected ? "purple-gradient text-white shadow-xl shadow-indigo-500/30 scale-105" : "hover:bg-white/10 text-white/40"
                         )}
                        >
                         {day}
                         {day === 10 && <div className="absolute bottom-1.5 w-1 h-1 bg-indigo-500 rounded-full" />}
                        </button>
                      );
                   })}
                </div>
             ...
             <div className="space-y-6">
                   ...
                   {timeSlots.map(slot => (
                     <button 
                        ...
                        className={cn(
                          ...
                          selectedTime === slot ? "bg-indigo-500 text-white border-indigo-500/50" : "bg-white/5 text-white/60 hover:border-indigo-500/40 hover:text-white"
                        )}
                      >
                        {slot}
                     </button>
                ...
                <button 
                  onClick={handleBook}
                  disabled={isBooking}
                  className="w-full purple-gradient py-5 rounded-2xl font-bold uppercase tracking-[0.2em] text-sm mt-8 shadow-2xl shadow-indigo-500/30 hover:scale-[1.02] active:scale-95 transition-all text-white disabled:opacity-50 disabled:hover:scale-100"
                >
        ...
        <aside className="space-y-6">
           <GlassCard delay={0.4} className="bg-indigo-600/5 group border-indigo-500/10 p-8 shadow-indigo-500/5">
              <div className="w-14 h-14 rounded-2xl bg-indigo-500/20 flex items-center justify-center mb-8 shadow-xl shadow-indigo-500/10 ring-1 ring-white/10">
              ...
              <div className="flex -space-x-4">
                ...
                <div className="inline-block h-10 w-10 rounded-full ring-4 ring-[#040D0E] bg-indigo-500 flex items-center justify-center text-[11px] font-bold text-white shadow-xl shadow-indigo-500/20 transition-transform hover:scale-110 cursor-pointer">+28</div>
              </div>
           </GlassCard>
              ...
              <div className="p-8 space-y-6 bg-[#040D0E]/80 backdrop-blur-md">
                <div className="space-y-1">
                   <div className="flex items-center gap-2 mb-2">
                     <span className="px-2 py-0.5 bg-indigo-500/10 text-indigo-400 text-[8px] font-bold uppercase tracking-widest rounded-full border border-indigo-500/20">Verified Professional</span>
                   </div>
                   <h3 className="text-2xl font-bold tracking-tight text-white">{viewingSpecialist.name}</h3>
                   <p className="text-indigo-400 text-xs font-bold uppercase tracking-widest">{viewingSpecialist.specialty}</p>
                </div>
                ...
                <div className="pt-6 border-t border-white/5">
                   <button 
                    ...
                    className="w-full purple-gradient py-4 rounded-xl font-bold uppercase tracking-widest text-xs text-white shadow-xl shadow-indigo-500/20 hover:scale-105 active:scale-95 transition-all"
                   >
                     Select Counselor
                   </button>
                   <button 
