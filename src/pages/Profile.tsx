...
export default function Profile({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  const [outdoorTime, setOutdoorTime] = useState<number>(30);
    ...
    if (user) {
      getDoc(doc(db, "users", user.uid)).then(docSnap => {
        if (docSnap.exists()) {
          ...
          if (data.sleepScore !== undefined) setSleepScore(data.sleepScore);
          if (data.outdoorTime !== undefined) setOutdoorTime(data.outdoorTime);
        }
      });
    }
  }, [user]);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 pb-12">
        ...
        <form onSubmit={handleUpdate} className="space-y-8">
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em] ml-1">Typical Sleep Score (1-10)</label>
                <input 
                  type="number"
                  min="1"
                  max="10"
                  value={sleepScore}
                  onChange={(e) => setSleepScore(Math.min(10, Math.max(1, parseInt(e.target.value) || 8)))}
                  className="w-full bg-white/5 border border-white/5 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-emerald-500/30 transition-all font-medium"
                />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em] ml-1">Typical Outdoor Time (minutes)</label>
                <input 
                  type="number"
                  min="0"
                  max="1440"
                  value={outdoorTime}
                  onChange={(e) => setOutdoorTime(Math.min(1440, Math.max(0, parseInt(e.target.value) || 30)))}
                  className="w-full bg-white/5 border border-white/5 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-emerald-500/30 transition-all font-medium"
                />
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {confirmLogout ? (
          <GlassCard className="p-6 border-red-500/20 bg-[#120505] flex flex-col justify-between gap-4 font-sans">
            <div>
              <p className="text-[10px] uppercase font-bold text-red-400 tracking-widest">Are you sure?</p>
              <p className="text-xs text-white/60 mt-1">Ready to end your session?</p>
            </div>
            <div className="flex gap-2 w-full">
              <button
                ...
                className="flex-1 bg-red-600 hover:bg-red-700 py-2 rounded-xl text-xs font-bold text-white transition-all cursor-pointer shadow-lg shadow-red-500/10"
              >
                Yes, Sign Out
              </button>
              <button
                ...
                className="flex-1 bg-white/5 hover:bg-white/10 py-2 rounded-xl text-xs font-bold text-white/60 hover:text-white transition-all cursor-pointer border border-white/5"
              >
                Cancel
              </button>
            </div>
