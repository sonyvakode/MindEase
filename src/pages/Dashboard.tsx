...
export default function Dashboard({ onNavigate }: { onNavigate?: (tab: string, state?: any) => void }) {
  ...
  if (!data) return (
    <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
      ...
      <p className="text-indigo-500/50 text-xs font-bold uppercase tracking-widest animate-pulse">Syncing Wellness Data...</p>
    </div>
  );
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <GlassCard className="flex items-center gap-6" delay={0.1}>
          <div className="w-16 h-16 rounded-full border-4 border-indigo-500/30 flex items-center justify-center relative shadow-[inset_0_0_10px_rgba(99,102,241,0.1)]">
          ...
          <div>
            ...
            <p className="font-bold text-indigo-400">{data.moodAverage > 7 ? 'Good' : data.moodAverage > 4 ? 'Moderate' : 'Low'}</p>
          </div>
        ...
        <GlassCard className="flex items-center gap-6" delay={0.2} onClick={() => onNavigate?.("appointments")}>
          <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center text-indigo-400">
            <Calendar className="w-6 h-6 shadow-[0_0_10px_rgba(99,102,241,0.3)]" />
          </div>
          <div>
            <p className="text-[10px] uppercase font-bold text-white/30 tracking-widest mb-1">Appointments</p>
            <p className="text-2xl font-bold">2</p>
          </div>
          <div className="ml-auto w-10 h-10 bg-indigo-400/20 rounded-xl flex items-center justify-center text-xl shadow-lg shadow-indigo-500/10">🗓️</div>
        </GlassCard>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[
            { icon: Smile, label: "Log Mood", tab: "mood", bg: "bg-indigo-500/10", text: "text-indigo-400" },
            { icon: Award, label: "Quizzes", tab: "resources", state: { view: "quizzes" }, bg: "bg-emerald-500/10", text: "text-emerald-400" },
            ...
            <button
              key={i}
              onClick={() => onNavigate?.(action.tab, (action as any).state)}
              className={cn(
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="grid grid-cols-1 gap-8">
          <GlassCard className="bg-indigo-600/10 border-indigo-500/20 hover:scale-[1.02] cursor-pointer transition-all" onClick={() => onNavigate?.("resources")}>
              ...
              <button className="text-xs font-bold text-indigo-400 flex items-center gap-2 hover:gap-3 transition-all">
