              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end gap-3">
                  ...
                  <button
                    ...
                    className="purple-gradient px-8 py-2 rounded-xl text-xs font-bold text-white shadow-xl shadow-indigo-500/20 disabled:opacity-50"
                  >
        ...
        {filters.map((f) => (
          <button
            ...
            className={cn(
              ...
              activeFilter === f ? "text-indigo-400" : "text-white/30 hover:text-white/60"
            )}
          >
            {f}
            {activeFilter === f && (
              <motion.div layoutId="community-tab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-indigo-500 shadow-[0_0_10px_#6366F1]" />
            )}
          </button>
        ))}
      </div>
              ...
              >
                <GlassCard className="group hover:border-white/10 transition-all cursor-default p-6 border-white/5">
                  <div className="flex items-center justify-between mb-6">
                            ...
                            <button 
                              ...
                              className={cn(
                                ...
                                followingAuthors.has(post.author) 
                                  ...
                                  : "text-white/20 hover:text-indigo-400"
                              )}
                            >
                    ...
                    <div className="bg-indigo-500/10 px-2 py-1 rounded text-[8px] font-bold text-indigo-400 uppercase tracking-widest">Supportive Community</div>
                  </div>
                    ...
                    <div className="flex items-center gap-6 pt-6 border-t border-white/5">
                      <button 
                        ...
                        className={cn(
                          ...
                          likedPosts.has(post.id) ? "text-indigo-400" : "text-white/30 hover:text-indigo-400"
                        )}
                        title={likedPosts.has(post.id) ? "Unlike post" : "Love this post"}
                      >
                        <Heart className={cn("w-4 h-4 transition-all", likedPosts.has(post.id) ? "fill-indigo-500" : "group-hover:fill-indigo-500")} />
                      ...
                      {auth.currentUser?.uid === post.authorId && (
                        deletingPostId === post.id ? (
                          <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/10 py-1 px-3 rounded-xl transition-all">
                            <span className="text-[10px] text-red-400 font-extrabold uppercase tracking-wide">Delete?</span>
                            <button 
                              onClick={(e) => {
                                e.stopPropagation();
                                handleDelete(post.id);
                              }}
                              className="text-[9px] font-extrabold uppercase bg-red-600 hover:bg-red-700 text-white px-2.5 py-1 rounded-lg transition-all cursor-pointer"
                            >
                              Yes
                            </button>
                            <button 
                              ...
                              className="text-[9px] font-extrabold uppercase bg-white/5 hover:bg-white/10 text-white/60 hover:text-white px-2.5 py-1 rounded-lg border border-white/10 transition-all cursor-pointer"
                            >
                              No
                            </button>
                          </div>
                          ...
                          <button 
                            ...
                            className="flex items-center gap-2 text-white/30 hover:text-red-400 transition-all text-xs font-bold uppercase tracking-widest group cursor-pointer"
                          ...
                          <div className="p-6 space-y-6">
                            ...
                            <form onSubmit={handleAddComment} className="relative mt-4">
                              <input
                                ...
                                className="w-full bg-black/20 border border-white/5 rounded-xl py-3 pl-4 pr-12 text-xs font-medium focus:outline-none focus:border-indigo-500/30 transition-all text-white placeholder:text-white/20"
                              />
                              <button
                                type="submit"
                                disabled={isCommenting || !newComment.trim()}
                                className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center text-indigo-400 hover:text-indigo-300 disabled:opacity-20 transition-all"
                              >
                                {isCommenting ? <div className="w-4 h-4 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin" /> : <Send className="w-4 h-4" /> }
                              </button>
                            </form>
                          </div>
                        </motion.div>
        ...
        <aside className="space-y-8">
           ...
           <GlassCard delay={0.4} className="bg-indigo-600/5 group border-indigo-500/10 p-8 shadow-indigo-500/5">
              ...
              <button 
                ...
                className="text-xs font-bold text-indigo-400/60 flex items-center gap-2 group-hover:text-indigo-400 group-hover:gap-3 transition-all"
              >
